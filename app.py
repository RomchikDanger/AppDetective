...
from forms import RegisterForm, LoginForm, InvestigationForm, EvidenceForm, CommentForm
...
def generate_story(image_path):
    ...
    promt = f'''напиши детективную историю по фотографии {filename}.
    добавь:
    - место преступления
    - подозреваемых
    - неожиданный поворот
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            password_correct = check_password_hash(user.password_hash, form.password.data)
...
def create_investigation():
    ...
    if form.validate_on_submit():
        ...
        investigation = Investigation(title=form.title.data, user_id=current_user.id, created_at=datetime.utcnow())
    ...
    if form.validate_on_submit():
        ...
        if file:
            ...
            if filename.endswith('.png') or filename.endswith('.jpg') or filename.endswith('.jpeg'):
                ...
                evidence = Evidence(investigation_id=investigation.id, photo_path=filepath, api_response_summary=story)

                db.session.add(evidence)
                db.session.commit()

                flash('Улика добавлена')
                return redirect(url_for('investigation', investigation_id=investigation.id))

            else:
                flash('Можно загружать только картинки')

    evidences = Evidence.query.filter_by(investigation_id=investigation.id).all()

    return render_template('investigation.html', investigation=investigation,
        evidences=evidences, form=form, comment_form=comment_form)
...
def like_investigation(investigation_id):
    existing_like = Like.query.filter_by(user_id=current_user.id, investigation_id=investigation_id).first()

    if not existing_like:
        like = Like(user_id=current_user.id, investigation_id=investigation_id)
...
def add_comment(investigation_id):
    ...
    if form.validate_on_submit():
        ...
        comment = Comment(text=form.text.data, user_id=current_user.id, investigation_id=investigation_id)
